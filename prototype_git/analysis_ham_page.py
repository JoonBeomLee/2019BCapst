...
import input_goal_page

analysis_ham_form = uic.loadUiType("./qt_design/analysis_ham.ui")[0]

class AnalysisHamWindow(QMainWindow, analysis_ham_form):
    ### 상단 버튼 start ###
    ...
    def detail_change(self):
        ...
        if(self.detail == "덤벨 데드리프트"):
            if(self.unit == None): self.lb_plot.setPixmap(QtGui.QPixmap("./qt_design/img/ham01_day.png")); return
            if(self.unit == "일"): self.lb_plot.setPixmap(QtGui.QPixmap("./qt_design/img/ham01_day.png"))
            if(self.unit == "월"): self.lb_plot.setPixmap(QtGui.QPixmap("./qt_design/img/ham01_month.png"))

    def unit_change(self):
        self.unit = self.list_unit.currentItem().text()

        if(self.unit == "일"):
            self.lb_plot.setPixmap(QtGui.QPixmap("./qt_design/img/ham01_day.png")); return
        else:
            self.lb_plot.setPixmap(QtGui.QPixmap("./qt_design/img/ham01_month.png")); return

    def btn_goal_clicked(self):
        goal_form = input_goal_page.InputGoalWindow()
        goal_form.exec_()
        set_goal = goal_form.goal

        self.lb_goal_num.setText(str(set_goal))
        self.progressBar.setValue(50 / int(set_goal) * 100)
    ### 추가 버튼 end ### 
    ...
    def __init__(self, dDay=None, profile_path=None):
        ...
        self.btn_goal.clicked.connect(self.btn_goal_clicked)
        # day 출력

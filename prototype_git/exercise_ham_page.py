...
### 운동 세부 페이지
import exercise_arm_page
import exercise_ham_in_page

ex_ham_form = uic.loadUiType("./qt_design/exercise_ham.ui")[0]

class ExerciseHamWindow(QMainWindow, ex_ham_form):
    ### 상단 버튼 start ###
    ...
    def btn_arm_clicked(self):
        self.dialog = exercise_arm_page.ExerciseArmWindow(self.dday, self.photo_path)
        self.dialog.show()
        self.close()
    ...
    def btn_ham_in_clicked(self):
        self.dialog = exercise_ham_in_page.ExerciseHamInWindow(self.dday, self.photo_path)
        self.dialog.show()
        self.close()
    ## 추가 버튼 end ###
    ...
    def __init__(self, dDay=None, profile_path=None):
        ...
        self.btn_ham_in.clicked.connect(self.btn_ham_in_clicked)
        ## 추가 버튼 end ###